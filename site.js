    ...
    *  Handle Input
    ...
    function onKeyDown(event) {
        ...
        if ( initialized == false ) {
            ...
            if (!inputFocused && ((event.key == 'j')||(event.key == 'left'))) {
                $('#inputBox').focus();
            ...
            if (!inputFocused && ((event.key == 'k')||(event.key == 'down'))) {
                $('#inputBox').focus();
                console.log("down");
                newPoint = new Point(currentPoint.x, currentPoint.y + yDistance);
                oldPoint = new Point(currentPoint.x, currentPoint.y);
            ...
            if (!inputFocused && ((event.key == 'l')||(event.key == 'right'))) {
                $('#inputBox').focus();
                newPoint = new Point(currentPoint.x + xDistance, currentPoint.y);
                oldPoint = new Point(currentPoint.x, currentPoint.y);
                direction = 'right';
            ...
            if (!inputFocused && event.key == 'o') {
                $('#inputBox').focus();
                newPoint = new Point(view.center.x, view.center.y);
                oldPoint = new Point(view.center.x, view.center.y);
            }
            
            if (!inputFocused && event.key == 's') {
                shape = "square";
            }
            if (!inputFocused && event.key == 'e') {
                shape = "ellipse";
            }

            if (!inputFocused && event.key == '3') {
            ...
            // History
            if (!inputFocused && event.key == 'b') {
                var oldText = historyBack.pop();
                historyForward.push(oldText);
                oldText.fillColor = 'red';
                currentPoint = oldText.point;
            }

            if (!inputFocused && event.key == 'f') {
            ...
            if (event.key == 'enter') {
                ...
                // historyBack.push(currentPoint);                
                if (historyBack.length != 0){
                    var newThing = historyBack.pop();
                    newThing.fillColor='black';
                    historyBack.push(newThing);
                }
                ...
                lastText = newText;
                lastText.fillColor = 'green';
                $("#inputBox").val("");
                $("#inputBox").blur();
                historyBack.push(lastText);
            }
        }
    }
